...
void Rainbow3DH::renderTo(CRGB* leds, uint32_t frameTime) {
  ...
  for (uint16_t i = 0; i < Params::NUM_PIXELS; i++)
    leds[i] = CHSV(((uint16_t)(currentHue - Mapping::points[i].z * step)) % 0xFF, 0xFF, 0xFF);
}
...
void Rainbow3DV::renderTo(CRGB* leds, uint32_t frameTime) {
  ...
  if (currentHue > 0xFF) currentHue -= 0xFF;
  for (uint16_t i = 0; i < Params::NUM_PIXELS; i++)
    leds[i] = CHSV(((uint16_t)(currentHue + Mapping::points[i].x * step)) % 0xFF, 0xFF, 0xFF);
}

void Solid::renderTo(CRGB* leds, uint32_t frameTime) {
  for (uint16_t i = 0; i < Params::NUM_PIXELS; i++)
    leds[i] = effectManager->getPalette()->sample((uint8_t)Mapping::points[i].z, 0xFF);
}
...
void BreathPal::renderTo(CRGB* leds, uint32_t frameTime) {
  float speedFactor = effectManager->getEffectSpeed();
  for (int i = 0; i < Params::NUM_PIXELS; i++)
    leds[i] = blend(CRGB(effectManager->getPrimaryColor()), effectManager->getPalette()->sample((uint8_t) Mapping::points[i].z, 0xFF), sin8_C(millis() * frameTime/5000.0 * speedFactor));
}
...
void DynamicPal::renderTo(CRGB* leds, uint32_t frameTime) {
  ...
  if (dyn_cnt > 10) {
    ...
    for (int i = 0; i < effectManager->getEffectIntensity() / 255.0 * Params::NUM_PIXELS; i++) {
      ...
      leds[index] = effectManager->getPalette()->sample((uint8_t)Mapping::points[i].z, 0xFF); 
    }
    dyn_cnt = 0;
  }
}
...
void Running::renderTo(CRGB* leds, uint32_t frameTime) {
  ...
  for (int i = 0; i < Params::NUM_PIXELS; i++)
    leds[i] = effectManager->getPalette()->sample((uint8_t)Mapping::points[i].z, sin8_C(Mapping::points[i].z * (effectManager->getEffectIntensity() >> 4) + current_phase));
}
...
void RunningSharp::renderTo(CRGB* leds, uint32_t frameTime) {
  ...
  current_phase += 0xFF * (frameTime / 20000.0) * speedFactor;
  for (int i = 0; i < Params::NUM_PIXELS; i++) {
    uint8_t b = sin8_C(Mapping::points[i].z * (effectManager->getEffectIntensity() >> 4) + current_phase);
    leds[i] = effectManager->getPalette()->sample((uint8_t)Mapping::points[i].z, (b & 0x80 ? 0xFF : 0x00));
  }
}

void FireFlicker::renderTo(CRGB* leds, uint32_t frameTime) {
  for (int i = 0; i < Params::NUM_PIXELS; i++) {
    leds[i] = effectManager->getPalette()->sample((uint8_t)Mapping::points[i].z, 0xFF - random8(effectManager->getEffectIntensity() >> 1));
  }
}
